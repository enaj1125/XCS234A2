...
class Linear(DQN):
    ...
    def initialize_models(self):
        ...
        self.q_network = nn.Linear( 
                    ...
                    * img_width
                    * n_channels
                    * self.config["hyper_params"]["state_history"], num_actions)
    ...
    def get_q_values(self, state, network="q_network"):
        ...
        ### START CODE HERE ###
        # the dim of input is (batch_size , img_height * img_width * n_channels * 
    ...
    def calc_loss(
        ...
        ### START CODE HERE ###
        not_terminated_or_truncated = ~torch.bitwise_or(terminated_mask, truncated_mask)
        mask = not_terminated_or_truncated.long()
        max_Q, _ = torch.max(target_q_values, 1)
        Q_samp = torch.add(rewards, mask * torch.mul(max_Q, gamma))

        actions = actions.type(torch.int64) 
        actions_index = actions.unsqueeze(-1)
        Q_val = torch.gather(q_values, 1, actions_index)

        loss = F.mse_loss(Q_samp, torch.squeeze(Q_val))

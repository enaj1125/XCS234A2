...
class NatureQN(Linear):
    ...
    def initialize_models(self):
        ...
        self.q_network = nn.Sequential(
            nn.Conv2d(n_channels * self.config["hyper_params"]["state_history"], 32, (8, 8), stride=4, padding=2), 
            nn.ReLU(), # how to correspond the Conv2 layer to the parameters 
            nn.Conv2d(32, 64, (4, 4), stride=2),
            nn.ReLU(),
            nn.Conv2d(64, 64, (3, 3), stride=1),
            nn.ReLU(),  # H', W', 
            nn.Flatten(),   # where flatten is needed? 
            nn.Linear(round(img_width/8 - 3) * round(img_height/8 - 3) * 64, 512),   # What is the # of input? H' * W' * 64
            nn.ReLU(),
       ...
       ## Questions: 
       ## What is self.config["hyper_params"]["state_history"] ? 
       ## target_network use the same sets of parameters 
    ...
    def get_q_values(self, state, network):
        ...
        ### START CODE HERE ###
        # state size torch.Size([1, 80, 80, 12]), input size (1, 12, 80, 80)
